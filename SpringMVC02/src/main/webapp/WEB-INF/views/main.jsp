  ...
  <script type="text/javascript">
	...
	function goContent(idx){ //상세보기=선택한 게시물의 접혀진(숨겨둔) 컨텐츠를 펼치는 함수 혹은 그 반대
		if($("#c"+idx).css("display")=="none"){		//접힌 컨텐츠를 펼치는 = 열릴 때
			...
			$.ajax({
				...
				url:"board/"+idx,
				type:"get",
								//data:{"idx":idx}, REST전송방식이기 때문에 없어도 됨
				dataType:"json",
	...
	function goDelete(idx){	//게시물 삭제 = 서버에 가서 삭제가 되어야하므로 ajax를 쓰는것임
		$.ajax({
			//url:"boardDelete.do",
			//type:"get",
			//data:{"idx":idx},	//파라미터로 넘기려면 이렇게 써야함
	...
	function goUpdate(idx){
		...
		$.ajax({
			//url:"boardUpdate.do",
			//type:"post",//여러개라서 포스트방식
			
			url:"board/update",
			type:"put",
			contentType: "application/json;charset=utf-8",	//JSON 형태로 정보를 주고 받기 때문에 타입설정 필요
			data:JSON.stringify/*JSON형태로 정보를 주고 받기 위해서 타입을 JSON으로 바꿔주는 역할임*/({"idx":idx, "title":title, "content":content}),
			success: loadList,
			error: function(){ alert("error");}
